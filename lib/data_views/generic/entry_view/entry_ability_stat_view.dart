      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(
                "${entry.name ?? ""}${entry.type == FeatureEntryType.abilityDc
                    ? " save DC"
                    : entry.type == FeatureEntryType.abilityAttackMod
                    ? " attack modifier"
                    : ""}",
                style: const TextStyle(fontWeight: FontWeight.bold),
              ),
              Text(
                "${entry.type == FeatureEntryType.abilityDc ? " = 8" : ""}${entry.attributes?.map((x) => " + ${x.name} modifier").join()} + Proficiency Bonus",
              ),
            ],
          ),
        ],
